...
import { Megaphone, MousePointer2, ClipboardList, ShoppingCart, Trophy, Trash2, Users, TrendingUp } from 'lucide-react';
...
export function FunnelNode({ id, data, selected }: NodeProps<FunnelNodeData>) {
  ...
  return (
    <div className={`
      min-w-[180px] bg-[#1f2028] border-2 rounded-xl overflow-hidden transition-all duration-500
      ${selected ? `${config.color} ${config.glow} scale-105` : 'border-white/10 shadow-lg'}
    `}>
      <div className={`p-2 ${config.bg} border-b border-white/5 flex items-center justify-between`}>
        ...
        {selected && (
          <button onClick={(e) => { e.stopPropagation(); deleteNode(id); }} className="text-white/40 hover:text-red-400"><Trash2 className="w-3 h-3" /></button>
        )}
      </div>

      <div className="p-4 flex flex-col gap-2">
        <h3 className="text-sm font-medium text-white/90">{data.label}</h3>

        {hasTraffic && (
          <div className="flex items-center gap-2 mt-1">
            <div className="flex flex-col">
              <span className="text-[10px] text-gray-500 uppercase font-bold">Acessos</span>
              <span className="text-xs font-mono text-white">{data.stats.views.toLocaleString()}</span>
            </div>
            <div className="h-4 w-[1px] bg-white/10 mx-1" />
            <div className="flex flex-col">
              <span className="text-[10px] text-gray-500 uppercase font-bold">Conv.</span>
              <span className={`text-xs font-mono ${config.color.replace('border-', 'text-')}`}>{data.stats.conversions}%</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="h-1 w-full bg-white/5 mt-auto">
        <div 
          ...
          style={{ width: hasTraffic ? `${data.stats.conversions * 2}%` : '0%' }}
        />
      </div>
